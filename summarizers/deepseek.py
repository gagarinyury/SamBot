...
Используйте подходящие эмодзи для каждого раздела и пункта."""
                    ...
                    return """Проанализируйте эту расшифровку YouTube и создайте подробное резюме на русском языке.

{content}

КРИТИЧНО ВАЖНО: Ваш ответ должен быть ОБЫЧНЫМ ТЕКСТОМ без markdown заголовков (без ###, ##, #). 

ИСПОЛЬЗУЙТЕ ТОЛЬКО ТАКОЙ ФОРМАТ:
- Первая строка = главная мысль видео в 1-2 предложениях
- Потом перечислите ключевые моменты обычными пунктами (• или 1. 2. 3.)
- В конце сделайте краткий вывод

НЕ ИСПОЛЬЗУЙТЕ:
- ### Заголовки (это сломает форматирование!)  
- ** жирный текст в заголовках
- Сложную структуру

ПРИМЕР ПРАВИЛЬНОГО ФОРМАТА:
Безумный мёд из Непала — это редкий галлюциногенный мёд, который добывают с риском для жизни на высоких скалах.

Ключевые моменты:
• Уникальность мёда и его свойства: содержит токсины рододендрона, вызывает галлюцинации
• Опасность добычи: сбор происходит на скалах высотой 100+ метров без страховки
• Культурный контекст: традиция народа гурунгов передается 7500 лет

Выводы: традиция находится под угрозой исчезновения из-за опасности и коммерциализации."""
            else:
                if request.summary_length == SummaryLength.BRIEF:
